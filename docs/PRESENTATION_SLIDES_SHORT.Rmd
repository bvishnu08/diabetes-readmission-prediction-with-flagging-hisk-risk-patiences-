...
**📍 Where Used:** First step in our pipeline, called by `scripts/run_train.py`
</div>
...
df = pd.read_csv(config.raw_data_path)  # ← IMPORTANT: This loads the data!
...
# Show first 10 rows as an interactive table (this is your "before" picture)
DT::datatable(head(raw, 10),
...
# Show a small sample from the cleaned training data
DT::datatable(head(train, 10),
             options = list(pageLength = 5, scrollX = TRUE),
...
if ("readmitted_binary" %in% names(train)) {
  ...
  ggplot2::ggplot(counts, ggplot2::aes(x = factor(readmitted_binary), y = n, fill = set)) +
...
**🎯 Purpose:** Find the best threshold to catch 80% of high-risk patients  
**📍 Where Used:** Called during model training to optimize for recall
</div>

```{python threshold-tuning, eval=FALSE, echo=TRUE}
# ============================================================
# ⭐ CRITICAL CODE: THRESHOLD TUNING FOR 80% RECALL
# ============================================================
...
def tune_threshold_for_recall(y_true, y_proba, target_recall=0.80):
    ...
    for threshold in thresholds:
        ...
        y_pred = (y_proba >= threshold).astype(int)  # ← KEY: Lower threshold = more flags
        ...
        if recall >= target_recall and recall > best_recall:
            best_recall = recall
            best_threshold = threshold  # ← Save this as our best threshold!
...
<div class="highlight-box">
**📁 File Location:** `src/feature_selection.py`  
**🎯 Purpose:** Pick the top 10 most important features  
**📍 Where Used:** Called before training Logistic Regression model
</div>

```{python feature-selection, eval=FALSE, echo=TRUE}
# ============================================================
# ⭐ CRITICAL CODE: FEATURE SELECTION USING MUTUAL INFORMATION
# ============================================================
...
def select_features(X_train, y_train, k=10):
    ...
    selector = SelectKBest(
        score_func=mutual_info_classif,  # ← KEY: This scores each feature
        k=k                              # ← KEY: We want top 10 features
    )
    
    # Fit the selector to our training data
    # This calculates MI scores for all features and picks the best ones
    X_selected = selector.fit_transform(X_train, y_train)  # ← Does the selection!
    
    # Get the names of selected features (so we know which ones were picked)
    selected_features = X_train.columns[selector.get_support()]  # ← Top 10 features!
...
# - ✅ Number of medications (high MI score - very predictive!)
# - ✅ Time in hospital (high MI score - important!)
# - ❌ Patient ID (low MI score - not useful for prediction)
```
...
<div class="highlight-box">
**📁 File Location:** `src/preprocess.py`  
**🎯 Purpose:** Clean messy data with missing values  
**📍 Where Used:** First step in pipeline, called by train_test_split_clean()
</div>

```{python preprocessing, eval=FALSE, echo=TRUE}
# ============================================================
# ⭐ CRITICAL CODE: DATA PREPROCESSING - HANDLING MISSING VALUES
# ============================================================
...
def basic_clean(df, cfg):
    ...
    df = df.replace('?', np.nan)  # ← Converts '?' to NaN
    ...
    df = df.dropna(thresh=threshold, axis=1)  # ← Drops bad columns
    
    # STEP 4: Fill in remaining missing values
    # WHY: Models need complete data - we fill missing values with most common value
    imputer = SimpleImputer(strategy='most_frequent')  # ← Use most common value
    df_imputed = imputer.fit_transform(df)  # ← Fills in missing values!
    
    return df_imputed  # ← Return clean data ready for modeling
...
<div class="highlight-box">
**📁 File Location:** `src/model.py`  
**🎯 Purpose:** Build two different models - one simple, one powerful  
**📍 Where Used:** Called by src/train.py to create both models
</div>

```{python two-models, eval=FALSE, echo=TRUE}
# ============================================================
# ⭐ CRITICAL CODE: BUILDING TWO MODELS - SIMPLE + POWERFUL
# ============================================================
...
xgb_model = XGBClassifier(
    n_estimators=100,     # ← Number of trees (more = more complex)
...
#   → Use Logistic Regression (can explain feature importance)
#
# Scenario 2: Need best possible prediction accuracy
#   → Use XGBoost (more accurate, even if harder to explain)
#
...
<div class="highlight-box">
**📁 File Location:** `scripts/run_train.py`  
...
config = Config()  # ← Knows where everything is stored
...
train_models(config)  # ← ONE LINE TO RULE THEM ALL!
...
#    ↓ [train.py - threshold tuning]
# 5. Tune Thresholds (80% Recall Target)
#    ↓ [train.py - save models]
...
from sklearn.metrics import (
    ...
    recall_score,       # ⭐ MOST IMPORTANT: How many high-risk did we catch?
...
def evaluate_model(y_true, y_pred, y_proba):
    ...
    - Recall: "Of all high-risk patients, how many did you catch?" ⭐
    ...
    metrics = {
        ...
        # ⭐ RECALL: THE MOST IMPORTANT METRIC!
        # "Of all high-risk patients, how many did we catch?"
        'Recall': recall_score(y_true, y_pred),  # ← THIS IS WHAT WE OPTIMIZE FOR!
...
**Key Findings:**
- ✅ **Both models hit our 80% recall target** - We catch 80% of high-risk patients!
...
def predict_risk(patient_data):
    ...
    lr_model = joblib.load('models/logreg_selected.joblib')  # ← Load Logistic Regression
    xgb_model = joblib.load('models/xgb_selected.joblib')    # ← Load XGBoost
    
    # STEP 2: Get predictions from both models
    # Models return probabilities (0 to 1), not just Yes/No
    lr_prob = lr_model.predict_proba(patient_data)[0][1]  # ← LR probability
    xgb_prob = xgb_model.predict_proba(patient_data)[0][1]  # ← XGBoost probability
    
    # STEP 3: Convert probability to clinical interpretation
    # We use the threshold we tuned earlier (0.35 for 80% recall)
    threshold = 0.35  # ← This is the threshold we found during training!
    
    # Convert probability to "HIGH RISK" or "LOW RISK"
    # This is what doctors see - not numbers, but clear categories!
    risk_level = format_risk_level(lr_prob, threshold)  # ← Clinical interpretation
    
    return risk_level, lr_prob, xgb_prob  # ← Return easy-to-understand result
